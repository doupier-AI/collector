...
当前源码已经提供本地 WebUI、Node API、SQLite 持久化、统一研究节点与三类关系边、树和关系图导航、连续正文与后台语义切片、研究恢复、文件导入与阅读、模型设置、运行记录、联网双路径、浏览器扩展及自动化测试。确认式融合和自动融合尚未实现；正文唯一事实源与统一研究地图仍按当前路线迁移。当前能力状态、基线、活动父 Issue 与待产品决定见 [项目现状](docs/PROJECT.md)。
