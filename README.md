...
- Chat 输入与文件导入；
- 当前内容阅读与选区引用胶囊（在此追问 / 深入研究这段）；
- 统一研究节点树与全屏树导航；
- 用户标记与笔记、标记列表和来源位置返回（见 issues #12/#13）；
- 外部模型供应商配置、流式生成与失败恢复；
...
快速演示时使用 `npm.cmd run launch:mvp`。该命令复用真实 WebUI、研究会话、SQLite、文件导入、阅读和刷新恢复，仅把研究回答切换为无需网络与密钥的确定性本地模拟；每条模拟回答都永久显示“本地演示回答｜非真实 AI｜未联网检索”。正式真实模型验收使用本机已保存的模型配置；真实模型路径的最新验证限制见 [项目开发记录](docs/PROJECT_DEVELOPMENT_RECORD.md)。演示模式与正式模式具有不同运行指纹，切换命令时启动器会安全更换服务，不会误复用另一种模式。
