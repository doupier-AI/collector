...
test("两节一次成功：每节恰一次 expand 调用，正文完整拼接", async (t) => {
  ...
  const provider = makeProvider({
    expandCalls,
    expandScript: [
      () => ({ content: "## 起源\n\n第一节正文，论述起源。", finishReason: "stop" }),
      () => ({ content: "## 实践\n\n第二节正文，论述实践。", finishReason: "stop" }),
    ],
  });
  ...
  assert.match(expandCalls[1]?.writtenSoFar ?? "", /第一节正文/, "后续节只接收已生成的干净前文");
  ...
  assert.ok(!store.getResearchMessage(outputMessageId)?.content.includes("[["));
  store.close();
});
...
test("单节截断续写一次成功：恰两次调用、第二次带 continuation 断点", async (t) => {
  ...
  const provider = makeProvider({
    ...
    expandScript: [
      ...
      () => ({ content: "## 起源\n\n第一节正文被截断在中途", finishReason: "length" }),
      (call) => ({ content: "，续写补全第一节并使用 RAG。", finishReason: "stop" }),
      () => ({ content: "## 实践\n\n第二节正文。", finishReason: "stop" }),
    ],
  });
  const { store, service } = await makeService(t, provider);
  ...
  assert.ok(expandCalls[1]?.continuation?.priorSectionContent.includes("第一节正文"), "续写携带干净断点前文");
  const content = store.getResearchMessage(outputMessageId)!.content;
