...
import { CaptureService, ResearchSessionService, SqliteStore, citedGroundingSources, formatFinalWriterEvidence, type ResearchGenerationProvider } from "@collector/api";
  ...
  const provider: ResearchGenerationProvider = {
    ...
    async prepareGrounded() {
      return {
        ...
        status: "grounded", queries: [], sources: [], citations: [],
      };
    },
  };
  const service = new ResearchSessionService(harness.store, { provider, autoRunTasks: false });
