...
当前 WebUI 已经实现核心闭环 MVP 并进入阶段 H：统一研究节点页与全屏树导航（H1/H2）、选区引用胶囊与双模发送（H4a，旧选区智能窗口退役）、术语检测数据层（H3a）、父链上下文组装（H5a）。当前四级验证基线（Node 全量、WebUI 全量、Playwright e2e、项目检查）以 `docs/PROJECT_DEVELOPMENT_RECORD.md` §5 为准，不在本文件维护计数。
...
右上“稍后再学”图标控制右侧栏；当前右侧栏只显示“暂无内容”，不伪造尚未实现的保存能力。（H4a 起稍后再学入口退役，右侧区域保持占位；标记与笔记入口在选区胶囊上重建，见 issues #12/#13。）
...
| 4 | 稍后再学 | ~~右侧抽屉、优先级和弱重现~~ UI 入口随 H4a 退役；数据底座语义调整为标记 + 笔记，交互由 issues #12/#13 重建 | AppShell 保留按需右侧区域，不常驻空侧栏 |
...
| 7 | AI 弱标记 | 低注意力概念标记和按需解释 | 数据层已完成（H3a，`aba0ddc`：术语位置契约 + 确定性检测服务）；渲染与悬停生长（H3b/H3c）未实施 |
...
缺少真实浏览器证据、接口核对或刷新恢复验证时，只能记录为部分完成或阻塞，不能写成 WebUI 阶段完成。阶段事实同步到 `docs/PROJECT_DEVELOPMENT_RECORD.md`，长期有效规则同步到对应产品、架构、验收或实施指导；同一事实不维护平行状态文件。
