...
test("全局研究图谱：两个会话的根节点进入同一真实观察结果，刷新后保持", async ({ page }, testInfo) => {
  ...
  await expect(page.getByTestId("global-map-canvas").getByRole("link", { name: /全局地图测试一/ })).toBeVisible();
  await expect(page.getByTestId("global-map-canvas").getByRole("link", { name: /全局地图测试二/ })).toBeVisible();
  ...
  await page.reload();
  const firstNode = page.getByTestId("global-map-canvas").getByRole("link", { name: /全局地图测试一/ });
