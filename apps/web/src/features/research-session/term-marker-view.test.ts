...
function messageWith(content: string, status: ResearchMessageRecord["status"] = "completed") {
  return makeMessage({ id: "answer", role: "assistant", content, status });
}
...
describe("当前正文 sidecar 弱标记", () => {
  it("只返回与当前正文版本和绝对范围逐字一致的记录", () => {
    ...
    const message = messageWith(content);

    expect(currentBodyTermMarkers(message, [valid, stale, wrongRange], researchBodyVersionId(message.id, content))).toEqual([valid]);
  });
  ...
  it("流式正文只追加时保留已闭合提及，完成后的旧版本记录则失效", () => {
    ...
    expect(currentBodyTermMarkers(messageWith(appended, "streaming"), [marker])).toEqual([marker]);
    expect(currentBodyTermMarkers(messageWith(appended, "completed"), [marker])).toEqual([]);
  });
});
