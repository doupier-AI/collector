...
/** 研究图谱只保存这一项入口偏好；坐标、筛选和专注不在这里持久化。 */
export function ResearchMapSettingsPage() {
  ...
  if (value === null && !error) return <main className="page" aria-label="节点图谱"><p className="settings-status">正在读取节点图谱设置…</p></main>;
  return <main className="page" aria-label="节点图谱">
    <h1 className="page__title">节点图谱</h1>
    <div className="settings-form__field">
      <label className="settings-form__label settings-form__label--toggle">
        <input type="checkbox" checked={value ?? false} disabled={saving} onChange={(event) => void change(event.target.checked)} />
        从节点正文进入时默认专注父子树
      </label>
