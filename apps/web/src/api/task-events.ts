...
/** 可注入的 EventSource 最小接口，便于单元测试替换。 */
...
export interface TaskEventStreamOptions {
  ...
  getTask: (taskId: string) => Promise<ResearchTaskRecord>;
  /** 已按事件 id 去重后的事件回调。 */
  onEvent: (event: ResearchTaskEvent) => void;
  /** 任务状态回调：终态确认、轮询更新。 */
  onTask: (task: ResearchTaskRecord) => void;
...
export type TaskEventStreamMode = "streaming" | "polling" | "closed";

export interface TaskEventStream {
  close(): void;
  /** 页面恢复可见时调用：轮询中立即同步一次；流式中立即按游标重连。 */
  syncNow(): void;
  readonly mode: TaskEventStreamMode;
...
function parseTaskEvent(raw: Event): ResearchTaskEvent | undefined {
  const data = (raw as MessageEvent<string>).data;
  if (typeof data !== "string") return undefined;
  try {
    const parsed = JSON.parse(data) as ResearchTaskEvent;
 ...
 *
 * 行为契约（与后端一致）：
 * - 服务端每次连接先发送 snapshot（无 id），再补发游标之后的持久化事件；
 * - 用事件 id 去重并保存内存游标；重连时用 ?after= 续传（浏览器原生重连自动带 Last-Event-ID）；
 * - completed / failed 后关闭连接，并 GET 任务终态确认；
 * - 连续失败超过上限后回退到 getTask 轮询，不丢已显示内容；
 * - close() 之后不再触发任何回调。
 */
export function connectTaskEvents(options: TaskEventStreamOptions): TaskEventStream {
  ...
  function handleNamedEvent(raw: Event): void {
    ...
    const event = parseTaskEvent(raw);
    if (!event) return;
  ...
  async function finishWithTerminalTask(): Promise<void> {
    ...
    clearScheduled();
    try {
      // completed / failed 后关闭连接并获取最终任务状态
