...
describe("connectImportEvents", () => {
  ...
  it("认证确认未返回时达到重连上限，晚到的 401 仍立即停止", async () => {
    ...
    const pollingRequest = new Promise<ResearchImportTaskRecord>(() => undefined);
  ...
  it("认证确认未返回时达到重连上限，晚到的终态仍立即收口", async () => {
    let resolveAuthorizationCheck!: (task: ResearchImportTaskRecord) => void;
    const pollingRequest = new Promise<ResearchImportTaskRecord>(() => undefined);
    const completed = makeImportTask({ id: "import-task-1", status: "completed" });
    const { tasks, getTask, source, stream } = createHarness();
    getTask
  ...
  it("轮询遇到 401 时停止并上抛错误，不循环请求", async () => {
    ...
    getTask
      .mockResolvedValueOnce(makeImportTask({ id: "import-task-1", status: "running" }))
