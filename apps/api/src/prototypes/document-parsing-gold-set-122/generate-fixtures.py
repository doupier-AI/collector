...
def positive_checks(name):
    labels={
        ...
        "hybrid-technical-report":["封面、来源声明、目录、原生正文/表图、附录和页映射作为同一报告完整保留。","目录、章节标题与页映射让读者能跨原生页、横向页和扫描页理解报告结构。","原生图表、长表、横向宽表与扫描页分别说明自己的页级和对象级复核范围。","第 6、12 页无 OCR 时页级失败；只有保留页图并明确披露缺口，整份报告才可降级为部分成功；不得伪装成原生精确文字。"],
...
def gold_set():
    return {"schemaVersion":2,"prototype":True,"issue":122,"decisionStatus":"confirmed","decidedOn":"2026-08-25","decisions":{"sampleSet":"approved-seven-file-minimum","layerWeights":{"documentValue":50,"diagnosticAnchors":50},"imageOnlyPageWithoutOcr":"failure","mixedDocumentWithoutOcr":"partial-only-if-page-image-preserved-and-gap-disclosed"},"question":"Can a candidate preserve research value and disclose loss, rather than merely export text?","scoring":{"candidate":False,"weights":{"documentValue":50,"diagnosticAnchors":50},"sampleGroups":{"core-research":70,"office-regression":10,"negative":20},"successRule":"Total >= 85; each core-research sample >= 75; each core-research documentValue >= 80; zero critical violations."},"criticalViolations":["fabricated_text","false_precise_locator","negative_reported_success","silent_content_loss"],"externalBenchmarks":external_benchmarks(),"samples":[
...
def verify(gold):
    ...
    expected_decisions={"sampleSet":"approved-seven-file-minimum","layerWeights":{"documentValue":50,"diagnosticAnchors":50},"imageOnlyPageWithoutOcr":"failure","mixedDocumentWithoutOcr":"partial-only-if-page-image-preserved-and-gap-disclosed"}
    if gold.get("decisionStatus")!="confirmed" or gold.get("decisions")!=expected_decisions: raise RuntimeError("confirmed product decisions are missing or inconsistent")
    if gold["scoring"].get("candidate") is not False or gold["scoring"].get("weights")!=expected_decisions["layerWeights"]: raise RuntimeError("confirmed scoring weights must be documentValue 50 and diagnosticAnchors 50")
    masked_object_score=100*.5+50*.5
    masked_overall_score=masked_object_score*.7+100*.3
    if masked_object_score!=75 or masked_overall_score>=85: raise RuntimeError("50/50 scoring must prevent a half-lost core object layer from reaching success")
    ...
    required_ui_markers=("已确认的评分规则","整份文档研究价值 50% · 对象级诊断锚点 50%","document.score * .5 + anchors.score * .5","无 OCR 页级失败","保留页图但无 OCR")
    stale_ui_markers=("候选权重，待裁决","部分/失败仍待裁决","部分还是失败仍等你裁决")
    if any(marker not in prototype_html for marker in required_ui_markers) or any(marker in prototype_html for marker in stale_ui_markers): raise RuntimeError("prototype UI does not reflect the confirmed decisions")
    for item in gold["samples"]:
    ...
    expected_hybrid_rule={"status":"resolved","passCondition":"Accurate OCR is returned with OCR provenance and uncertainty disclosed.","noOcrPageOutcome":"failure","mixedDocumentOutcome":"partial-success-only-when-page-image-is-preserved-and-native-text-gap-is-explicitly-disclosed"}
    if hybrid_gold.get("expectedOutcome")!="conditional" or hybrid_gold.get("decisionDependency")!=expected_hybrid_rule: raise RuntimeError("hybrid document decision rule is unresolved or inconsistent")
    ...
    if any("without OCR this page-level anchor fails" not in anchor.get("structureExpectation","") for anchor in raster_anchors.values()): raise RuntimeError("each no-OCR image-only page must fail at page level")
