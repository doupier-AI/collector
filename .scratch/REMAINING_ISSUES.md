...
> 更新于 2026-06-15 | 72 tests passing | GUI smoke passing | 所有已知问题已修复
...
| A | 近期整理聚类永远为空 | 添加 cluster_materials 步骤到 organizeRecent 工作流 |
| B | 专题文档生成是垫片实现 | build_outline 和 draft_sections 接入 DeepSeek 网关，无网关时回退垫片 |
| C | 文档版本查询端点 404 | 实现 GET /v1/topics/{id}/documents、GET /v1/documents/{id} 路由 |
| D | 近期整理快照不持久化 | SQLite 持久化 + 测试验证通过 |
| E | 缺少端点 | 实现 /v1/data-paths、/v1/ai-configuration 端点 |
| F | 浏览器扩展未端到端验证 | 构建产物验证通过 |
| G | Electron 桌面端 GUI 待验证 | GUI smoke test 全部通过 |

## 全部修复完成

所有 Issue 均已修复。72 个单元测试 + GUI smoke test 全部通过。