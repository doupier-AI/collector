...
核心流程 MVP 可体验已于 2026-07-21 生效。当前推进阶段 E（可信研究能力）：按需联网搜索进入真实研究流程，AI 关键陈述具备可验证的行内引用与来源返回。
...
| E1 | 搜索适配层与搜索轨迹持久化（迁移 v20） | 已完成 | （本提交后回填） | 四级 |
| E2 | 三场景生成集成与引用校验持久化 | 未开始 | — | 四级 |
| E3 | 动态范围说明、行内引用胶囊与来源预览 WebUI | 未开始 | — | 二级 + 真实浏览器 + e2e |
...
7. **搜索是任务内的一个步骤（阶段 E）**：联网搜索不新增第四条 SSE 客户端，作为研究生成任务内的步骤复用既有任务事件管线（新增 `search_started / search_completed / search_failed` 事件）；搜索适配层独立可替换（SearXNG 兼容接口为首个实现，`COLLECTOR_SEARXNG_URL` 支持自带后端），复用既有公网目标校验与重定向逐跳再校验；任何搜索失败都降级为仅用当前材料并如实提示，绝不阻断生成。
8. **引用标记协议与服务端校验（阶段 E）**：提供来源时模型在需要证据的陈述后标注 `[n]`；生成完成后服务端用 `parseWebCitations` 解析、校验（编号越界即丢弃）、剥离标记并把位置换算到干净文本的段落块内偏移，干净内容覆写消息、引用落库；结构性失败为可重试失败，绝不静默造假。模型自主判断搜索时机属后续增强。
...
用户已确认的产品决定：全部 AI 内容生成场景（Chat 回答、深入研究第一轮、分支追问）自动联网搜索，选区分析不联网；搜索失败降级为仅用当前材料并如实提示，v1 不提供用户开关。

**E1（已完成）**：契约新增搜索轨迹记录（`WebSearchRecord` / `WebPageReadRecord` / `WebCitationRecord`）、任务范围 `WebSearchScope`、纯函数 `parseWebCitations` 与 `deriveSearchQuery`；迁移 v20 三张轨迹表（`web_searches` / `web_page_reads` / `web_citations`）与 clearAllData 补齐；`apps/api/src/web-search.ts` 实现 `SearchProvider` 接口、`SearxngSearchProvider`（故障切换、公网校验复用 `fetchPublicResource`、搜索响应限 JSON）、`FakeSearchProvider` 与 `WebSearchService`（失败恒降级不抛错）；`fetchPublicResource` 导出并扩展 JSON 与显式配置后端的非公网首跳例外。验证：Node 测试 230/230（新增 32 项），四级重建基线全部通过。详见开发记录 3.22。

**E2（未开始）**：`processTask()` 在 claim 之后、生成之前接入 `WebSearchService`（重试复用本任务已完成的搜索记录）；扩展任务事件联合与 `ResearchGenerationRequest.webSources`；真实模式适配器与网关方法带来源提示（`deep-research-v2` / `research-chat-v2`）；生成完成后解析校验引用、覆写干净消息内容、持久化引用与任务范围；会话 / 分支视图联接引用。

**E3（未开始）**：动态范围说明（页面级 + 消息级）、实时搜索步骤进度、零文本引用标记与段落内渲染（与选区高亮共存）、来源预览临时浮层与参考来源列表、选区无扰动回归；e2e harness 注入确定性假搜索，Playwright 覆盖引用链、降级路径与刷新稳定性。

**收尾（阶段 E）**：镜像既有真实模型验收 harness 注入真实 SearXNG（`COLLECTOR_SEARXNG_URL`）做一次性验收：真实搜索 + 真实引用 + 降级路径 + SQLite 轨迹核对（无凭证入库）。
...
- 深入研究第一轮材料范围已确认：阶段 E 起全部 AI 内容生成场景自动联网搜索，搜索失败降级为仅用当前材料并如实提示（2026-07-22 用户确认）；
