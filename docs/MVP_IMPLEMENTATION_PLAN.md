...
核心流程 MVP 可体验已于 2026-07-21 生效。当前并行推进两个方向：阶段 E（可信研究能力，代码已完成待真实验收）和阶段 F（联网搜索策略改进，调研已完成待启动第一阶段）。
...
| F1 | 搜索链路日志与查询改写（自建搜索可观测性） | 已完成 | `68b39be` | 二级（构建 + 全量测试 205/205） |
| F2 | web_search / web_fetch 工具拆分 + Agent 多轮工具调用循环 | 已完成 | `9952a02` | 三级（构建 + 全量测试 214/214，含 9 个 Agent 循环测试） |
| F3 | 多搜索后端可选（DDG / Tavily / SearXNG） | 已完成 | `efe0d73` | 三级（构建 + 全量测试 235/235，含 21 个搜索后端测试） |
...
10. **搜索改进分三阶段推进（阶段 F）**：第一阶段（F1）不改架构，仅加日志和查询改写，低成本修复 Bing 中文分词偏差问题；第二阶段（F2）把搜索变为 Agent 可多轮调用的独立工具，对标 DeerFlow 的自主搜索体验；第三阶段（F3）增加多后端支持，解除 Bing 单点依赖。详细背景和调研结论见 `docs/agents/search-optimization.md`。
...
### 阶段 F：联网搜索策略改进（调研完成，待启动）

当前自建搜索（Bing HTML 抓取）存在三个核心问题：中文分词导致搜索失真（如「什么是loop engineering」返回「什么」的词典解释）、零可观测性、架构僵化（单轮搜索，结果注入 prompt，模型只能被动接受）。详细分析见 `docs/agents/search-optimization.md`。

**F1（待启动）**：搜索链路日志 + 查询改写。

- 在 `web-search-agent.ts` 关键节点（`searchBing`、`fetchPageContent`、`runWebSearch`）加 `console.log`，输出 query、结果数、抓取成功/失败、总耗时；
- 在 `runWebSearch` 之前，用当前模型做一次轻量 query reformulation：用户原始消息 → 适合搜索引擎的关键词 → 再传给 Bing；
- 可额外增加一个 debug API 端点用于查询最近的搜索运行记录（可选，不作为 F1 必需交付）；
- 不改架构，不考虑多后端，不引入 Agent 循环。改动集中在 `web-search-agent.ts`。
- 验证：二级或三级（受影响文件 + 新增测试），真实浏览器检查搜索结果的日志输出。

**F2（待 F1 完成后启动）**：web_search / web_fetch 工具拆分 + Agent 多轮工具调用循环。

- 把 `web-search-agent.ts` 中的搜索和抓取拆为两个独立函数（或工具）：`web_search(query, maxResults)` 只搜不抓，`web_fetch(url)` 只抓不搜；
- 在 Agent 层面支持多轮工具调用循环（tool-use loop），让模型可以：搜 → 看结果 → 决定抓哪些 → 信息不够 → 换词再搜 → 反复直到满意；
- 需要框架层面的改造：当前 `ModelGateway.complete()` 是单轮 prompt→response 模式，且 `answerResearchConversation` 强制 `responseFormat: "json_object"` 阻止工具调用。需评估改造方案（Vercel AI SDK / 手写 ReAct / 原生 tool-use API）；
- 验证：三至四级，新增 Agent 循环的单元测试和集成测试。

**F3（待 F2 完成后启动）**：多搜索后端可选。

- 至少增加 DuckDuckGo（免费，无需 API Key）作为 Bing 回退；
- 可选增加 Tavily（AI 专用搜索 API，需 Key）；
- 后端选择通过配置切换，接口统一；
- 验证：三级，新增搜索后端的单元测试。
...
- **搜索改进（阶段 F，2026-07-23）**：F1（日志+改写）优先级已确认，等待启动；F2 Agent 循环的实现框架选择（Vercel AI SDK / 手写 ReAct / 原生 tool-use API）待调研当前项目依赖后确认；F3 Tavily API Key 的获取和管理是否需要待确认。
