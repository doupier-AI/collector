# Collector WebUI 前端实施指导

最后更新：2026-07-20

状态：当前前端实施基线

阅读规则：首次参与前端实施、本文发生重要变化或当前任务涉及对应章节时阅读；日常任务只加载相关产品、契约、源码和测试。

## 1. 指导目标

本文提供给负责 `apps/web` 的开发者。阅读本文后，可以在不重新整理全部产品讨论的情况下继续 Collector WebUI 开发。
...
相同开发者连续工作且本文未发生相关变化时，不重复读取上述长文档，只读取当前任务涉及的章节。
...
- 同一后端阶段连续补齐共享契约、迁移、持久化、接口、恢复和确定性联调条件，达到前端可以直接开发的门槛后再进入前端实现；前端不接收仅有契约说明或缺少真实可调用能力的半成品；
...
以上阻塞项未完成前，前端不自行引入 token URL 参数、浏览器持久化密钥、跨域绕过、假上传成功或前端伪流式输出。
...
缺少真实浏览器证据、接口核对或刷新恢复验证时，只能记录为部分完成或阻塞，不能写成前端阶段完成。阶段结果同步到 `docs/PROJECT_DEVELOPMENT_RECORD.md`，长期有效规则同步到对应产品、架构、验收或实施指导，不创建按开发角色往返维护的责任文件。
