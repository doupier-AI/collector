...
- **章节导航按真实收展状态让位（目标为消灭 #9，supersedes ADR-0016 的 `:has(.drawer--fixed)` 方案）**：`.slice-rail` 默认 `transform:translate(-100%,-50%)` 翻在正文左缘外侧，宽侧栏展开时伸进侧栏区域拦截 ⋯菜单点击。旧 `:has(.drawer--fixed)` 只看 DOM 存在、分不清侧栏真实宽度（64↔400px 跳变）。改为：AppShell 把左侧栏 `collapsed` 真实状态提升为 `.app-shell` 根 class（`app-shell--sidebar-open` / `app-shell--sidebar-collapsed`），让位规则 `.app-shell--sidebar-open .slice-rail`（收进正文左缘内侧 + `.page` padding-left 2.75rem）只在**展开态**生效；收起成 64px rail / 侧栏整隐 / 窄屏 overlay 时恢复翻出正文左缘外侧的悬停设计。scrollspy 几何决胜逻辑（观察者登记卡片 + scroll 驱动裁决 + 阅读线 35%）不变。
...
**Consequences:** 保留契约——`内容导航` nav、会话分组树/批量选择/重命名/移动/归档/删除、宽屏拖拽调宽（separator aria-valuemin/max/now）、设置聚合菜单四入口真实导航（AI 模型/融合/运行记录/回收站，`${title} 的菜单` aria-label）；`RAIL_WIDTH`/`DETAIL_WIDTH` 常量语义变为「rail 宽/展开默认宽」；rail 链接改用普通 `Link` + 手动 `aria-current`（「会话」激活是业务语义「在研究区任意路径」，NavLink 的 URL 前缀匹配表达不了）。组件测试（app-shell 收展/搜索/设置菜单、session-list 菜单 fixed 锚点）+ Playwright `session-management.spec.ts`（全流程、收展+设置菜单导航、⋯菜单 fixed 不漂移不裁剪）通过；视觉基线 8 张零回归（基线截图前收起侧栏，侧栏结构与 slice-rail 让位不落入截图，按 ADR-0012 免重拍）。#7/#10 保持原有完成证据；#9 的当时自动化结果已被后续用户复验推翻。

**2026-08-10 复验更正：** 用户在实际界面复验确认章节导航器仍与正文重叠。上述让位规则是已经采用的设计与实现方案，但不能证明 #9 已修复；原关闭说明中的该项验收结论作废，#54 功能上重新列为 ACTIVE，直到真实触发场景被复现、修复且进入浏览器回归测试。GitHub 当前仍为 closed：仓库连接可读取但无 Issue 写权限，待恢复权限后重新打开并补充复验说明。#54 属于 #51，与 #45 无关。
