# 自动融合现有实现（目标自动化已由 ADR-0024 取代）

> **目标产品规则由 ADR-0022 与 ADR-0024 取代（2026-08-13）**：本 ADR 继续记录当前已实现、默认关闭且可直接创建永久融合节点的旧能力，供迁移前核对。新节点模型由 AI 自主发现并自动创建研究地图 B 面中的临时融合节点，用户确认后才进入 A 面永久知识结构；不再授权 AI 直接创建永久融合节点。
