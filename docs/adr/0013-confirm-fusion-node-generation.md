# 确认式融合现有实现（目标确认流程已由 ADR-0024/0025 取代）

> **目标模型由 ADR-0022 扩展、ADR-0023 收缩，并由 ADR-0024 修订（2026-08-13）**：融合节点继续保持无父节点、来源可回溯和纯增量，但确认前已作为 B 面临时融合节点存在；确认提升同一节点身份、地址、位置和完整内容，不在确认时另建节点或重新生成正文。融合提议可包含两个以上来源，并按一条新综合区分，而非按节点对唯一。本 ADR 中“确认融合同时创建普通语义关联边”和“确认时创建融合节点”的行为只说明当前实现；目标模型不再写入普通语义边。
