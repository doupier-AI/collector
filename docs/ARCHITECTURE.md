...
状态：2026-07-18 本地 WebUI 当前架构。首个研究会话切片和 API 同源生产资源服务已实现，启动器生命周期等能力继续按本架构实施。
...
当前已实现 Node API、SQLite、共享契约、模型网关、文件解析、研究会话恢复、首个 WebUI 切片和生产构建同源服务。正式服务从专用 WebUI 构建目录读取资源，浏览器页面路由回退到 SPA 入口；`/v1` 路由继续使用本地认证和来源校验。

本地启动器、首次安全配对引导、完整 Web 凭证边界、文件导入和后续产品切片属于当前实施工作。
