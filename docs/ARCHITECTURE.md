...
状态：2026-07-18 本地 WebUI 当前架构。首个研究会话切片、API 同源生产资源服务、动态端口启动器与首次安全配对已实现。
...
当前已实现 Node API、SQLite、共享契约、模型网关、文件解析、研究会话恢复、首个 WebUI 切片、生产构建同源服务、动态端口启动器、重复启动复用和首次安全配对。正式服务从专用 WebUI 构建目录读取资源，浏览器页面路由回退到 SPA 入口；`/v1` 路由继续使用本地认证和来源校验。
