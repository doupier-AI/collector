...
状态：2026-07-31 本地 WebUI 当前架构。研究会话核心闭环、选区引用胶囊与双模发送（旧选区智能窗口 H4a 起退役）、深入研究（节点生长）、用户标记数据底座（交互重建中，issues #12/#13）、文件导入与阅读、动态端口启动器、安全配对、启动器版本替换与数据目录独占、受控关闭、供应商联网引用、Agent 自主搜索循环（Bing/DDG/Tavily/SearXNG）、WebUI 模型配置与凭证持久化、模型发现与按任务类型路由、统一研究节点树与全屏树导航（H1/H2）、术语检测数据层（H3a）、父链上下文组装（H5a）等已实现。
...
- 用户标记与笔记数据底座（交互重建中，issues #12/#13；旧稍后再学入口已退役）；
- 加载、空状态、恢复状态和错误状态；
- 键盘、焦点、响应式布局和减少动态效果。
...
WebUI 统一为节点页（`/research/:sessionId/node/:nodeId`）：中央显示当前节点内容，顶部来源条展示来路并可返回原内容原选区，顶栏按钮或快捷键 `t` 唤出全屏树导航（面包屑路径、键盘可达）。选区引用胶囊当前在输入框区域呈现，交互修订（issues #9–#11）将改为选区上方浮动。右侧区域当前为占位，不常驻稍后再学等内容。
