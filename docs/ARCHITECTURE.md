...
- 研究节点在既有 `record_json` 中保存下一次提交使用的输入偏好；生成任务在提交时冻结任务模式、深入研究上下文、模型用途与配置身份、联网后端策略和 thinking 有效值。自动重试、暂停与恢复复用该执行意图；用户主动重试、重新生成或编辑按当前设置创建新尝试。
- 普通任务视图只投影运行时结构化执行事件；供应商 `reasoning_content` 不持久化、不进入 API、界面、检索或导出，旧数据库记录不从普通路径投影。
