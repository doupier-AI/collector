...
状态：2026-07-31 本地 WebUI 当前架构。研究会话核心闭环、选区引用胶囊与双模发送（旧选区智能窗口 H4a 起退役）、深入研究（节点生长）、用户标记与笔记及标记列表返回（issues #12/#13）、文件导入与阅读、动态端口启动器、安全配对、启动器版本替换与数据目录独占、受控关闭、供应商联网引用、Agent 自主搜索循环（Bing/DDG/Tavily/SearXNG）、WebUI 模型配置与凭证持久化、模型发现与按任务类型路由、统一研究节点树与全屏树导航（H1/H2）、术语检测数据层（H3a）、父链上下文组装（H5a）等已实现。
...
- 浏览器文件选择与拖放上传；
- AI 回答和导入文档的统一阅读；
- 选区引用胶囊与双模发送（在此追问 / 深入研究这段）、来源高亮；弱标记数据层已就位（H3a），渲染未实施；
- 统一节点页、全屏树导航和来源线（深入研究二选一与研究分支界面已于 H2 退役）；
- 用户标记与笔记、标记列表和来源返回（issues #12/#13；旧稍后再学入口已退役）；
- 加载、空状态、恢复状态和错误状态；
- 键盘、焦点、响应式布局和减少动态效果。
...
WebUI 统一为节点页（`/research/:sessionId/node/:nodeId`）：中央显示当前节点内容，顶部来源条展示来路并可返回原内容原选区，顶栏按钮或快捷键 `t` 唤出全屏树导航（面包屑路径、键盘可达）。选区操作在选区上方显示浮动胶囊，可显式引用或标记并填写笔记；右侧“标记”栏展示选区摘要、笔记、来源节点和时间，点击项目通过 `?sel=` 返回原内容并恢复高亮，恢复后不自动创建引用。
...
运行版本兼容检查、旧版本安全更换和数据目录单服务独占改动已经提交；这些能力尚未完成真实旧服务替换与异常退出验收，不计入已完成的运行时验收边界。
...
- 模型凭证存储、读取、清理和不可用状态；
- 选区引用胶囊定位、键盘操作与双模发送（交互修订 issues #9–#11 重建定位与取消规则）；
- 用户标记与笔记、标记列表来源返回和 `?sel=` 恢复（issues #12/#13）。
