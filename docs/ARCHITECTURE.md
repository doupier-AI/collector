...
当前已实现 Node API、SQLite、共享契约、模型网关、文件解析、研究会话恢复、会话创建与消息提交幂等、首个 WebUI 切片、生产构建同源服务、动态端口启动器、重复启动复用和首次安全配对。正式服务从专用 WebUI 构建目录读取资源，浏览器页面路由回退到 SPA 入口；`/v1` 路由继续使用本地认证和来源校验。

已配对浏览器扩展保留 `43110` 回环适配入口，并与动态 WebUI 端口共享同一认证和领域服务。该入口不提供 WebUI，未配对请求仍被拒绝。

文件导入、WebUI 可见关闭入口和后续产品切片属于当前实施工作。
