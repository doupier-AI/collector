...
进行中：切片 D2（稍后再学栏目与来源返回 WebUI）已完成并提交，二级 + 真实浏览器 + e2e 验证全部通过（前端构建、WebUI 测试 185/185、Playwright Chromium 37/37）。核心闭环主链路（阅读当前内容 → 手动选区 → 选区智能窗口 → 深入研究 / 稍后再学 → 保留来源关系 → 返回原内容和原选区）已全部连通。收尾阶段（四场景真实模型 + 真实浏览器端到端验收与文档同步）已完成，满足 DEVELOPMENT_START.md 第 9.1 节全部条件，”核心流程 MVP 可体验”表述已启用。
...
收尾阶段（2026-07-21）重建的完整四级基线：

| 范围 | 结果 |
| --- | --- |
| Node 单元与集成测试 | 137/137 通过 |
| WebUI 客户端测试 | 185/185 通过 |
| Chromium 端到端场景 | 37/37 通过并自然退出 |
| Collector 项目检查 | 通过 |
| 真实云模型调用 | 已通过（DeepSeek deepseek-v4-flash，四场景验收 4/4） |

这是首次在同一阶段记录覆盖完整核心流程的真实模型 + 真实浏览器验收结果，满足 `DEVELOPMENT_START.md` 第 9.1 节全部条件，”核心流程 MVP 可体验”表述已正式生效。
