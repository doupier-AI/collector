...
阶段 H 并行三切片合并后（2026-07-30，§3.37）当前四级基线：

| 范围 | 结果 |
| --- | --- |
| Node 单元与集成测试 | 302/302 通过 |
| WebUI 客户端测试 | 238/238 通过 |
| Playwright e2e（假模型，真实 Chromium） | 46/46 通过（chromium 43 + chromium-nomodel 3），2 项暂停（旧面板稍后再学入口随 H4a 退役，待票据 08 恢复） |
| Collector 项目检查 | 本轮未执行（检查脚本路径缺失） |
...
- H4a（§3.37）起旧选区智能窗口退役，替换为引用胶囊与双模发送（在此追问 / 深入研究这段）；旧面板「稍后再学」UI 入口暂时移除，后端端点保留，新入口待票据 08；术语检测（H3a）与父链上下文（H5a）为地基层，尚未接入提示词与渲染；
- 离线演示回答不是产品能力证据；
