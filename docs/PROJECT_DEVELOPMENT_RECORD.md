...
阶段 H 并行三切片合并后（2026-07-30，§3.37）建立四级基线；选区交互修订一·A（2026-07-31，§3.38）为纯前端切片，按二级 + 真实浏览器 + e2e 验证，后端与契约未改动，Node 基线沿用：

| 范围 | 结果 |
| --- | --- |
| Node 单元与集成测试 | 302/302 通过（修订一·A 未改动后端，沿用 §3.37 基线） |
| WebUI 客户端测试 | 255/255 通过（修订一·A 后，含浮动胶囊定位 / 组件 / 捕获层状态机新增 17 项） |
...
- H4a（§3.37）起旧选区智能窗口退役，替换为引用胶囊与双模发送（在此追问 / 深入研究这段）；修订一·A（§3.38）起引用改为选区上方浮动胶囊显式触发，引用态与原生选区解耦（死循环修复），Escape 不再关闭选区或胶囊，取消方式唯一为点击选取以外区域；最短字符限制、`?sel=` 恢复后浮动胶囊呈现与窄屏 / 键盘 / 动效收口待修订一·B/#10 与修订一·C/#11；旧面板「稍后再学」UI 入口暂时移除，后端端点保留，新入口待修订二（issues #8/#12/#13）；术语检测（H3a）与父链上下文（H5a）为地基层，尚未接入提示词与渲染；
- 离线演示回答不是产品能力证据；
