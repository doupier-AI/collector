...
- D3（#30）已在后续 §3.48 完成并关闭；F2（#31）仍负责确认式融合节点，F3（#32）仍负责自动融合模式。本轮 F1 只建立相似性提议和语义关系边，不提前实现融合节点。
...
- 已完成并关闭：D2（#26）、D3（#30）、E2（#27）、E3（#28）、F1（#29）；#33 的 6 个预存 e2e 失败已在阶段前修复并关闭。
- 当前仍待推进：F2（#31，依赖 F1 / D1 / E2）、F3（#32，依赖 F2）。推荐下一轮先推进 F2 的融合节点边界设计与实现，再根据 F2 的融合节点契约安排 F3。
- 本轮没有新增 E3 数据库迁移；F1 独占迁移 v30，E2 / D2 / E3 均复用既有持久化结构。

### 3.48 阶段 I-D3：图交互收口与边筛选（issue #30，2026-08-02）

**用户可见结果**

- 桌面网状画布和窄屏关系列表都支持父子、语义相关、融合来源三类关系筛选；筛选只改变当前呈现，不改写底层图投影。
- 筛选后画布只保留当前节点和实际关系端点，方向键、Home/End 只在相关节点之间移动；全部关系关闭时仍保留当前节点并显示明确空态。
- 画布提供“打开父节点”和“返回当前页面”安全出口，保留已有平移、缩放、Escape 关闭、入口焦点回归和 reduced-motion 行为。

**实际改动与接口 / 数据变化**

- `useRelationships.ts` 新增共享边类型集合、active 边过滤、关系端点过滤和键盘候选派生；`RelationshipList.tsx` 与 `GraphCanvas.tsx` 共用同一呈现契约。
- `GraphCanvas.tsx` 增加可访问筛选控件（`aria-pressed`）、筛选后节点/边渲染、安全出口与过滤空态；`global.css` 增加桌面/窄屏筛选和安全出口样式。
- 组件测试覆盖筛选纯函数、投影不变、空筛选保留当前节点、画布和关系列表键盘/筛选/安全出口；Playwright 在浏览器路由中注入确定性三边投影，不写数据库。
- 不新增 HTTP API、数据库迁移、持久化字段，也未实现 F2/F3 融合节点能力。

**验证证据**

- 选择四级回归范围：D3 影响共享导航呈现、桌面/窄屏跨端路径和可访问键盘交互，因此除局部 WebUI 验证外执行项目构建与 Node 全量测试。
- `npm run build` 通过；WebUI 全量测试 37 个文件、316/316 通过；Node 全量测试 390/390 通过。
- 定向 Playwright Chromium 1/1 通过，覆盖三类边投影、筛选、筛选后键盘遍历、Enter 前置聚焦、缩放、平移、reduced-motion、窄屏列表一致性、控制台问题和横向溢出。
- 未执行 Chrome DevTools MCP 人类拟真测试，原因是用户未触发“进行一次人类拟真测试”；真实供应商联网验收继续按阶段 E 约定停止。

**未完成项与风险**

- F2（#31）仍负责确认式融合节点，F3（#32）仍负责自动融合模式；D3 只消费已有类型化边，不提前改变融合数据契约。
- Playwright 的语义相关和融合边使用路由注入的确定性投影，仅验证 WebUI 呈现契约，不替代 F2/F3 的后端数据闭环验收。

**下一步交接语**

- 已完成并关闭：D3（#30），阶段 I 的 D1、D2、E1、E2、E3、F1 均保持完成。
- 当前仍待推进：F2（#31，依赖 F1 / D1 / E2）→ F3（#32，依赖 F2）。后续不要与共享导航契约冲突地并行修改，先完成 F2 的融合节点持久化与用户确认路径，再安排 F3 自动融合模式。
