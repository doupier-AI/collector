...
收尾阶段已用真实 DeepSeek 模型 + 真实 Chromium 浏览器完成四场景端到端验收（场景一 Chat → 深入研究 → 返回 → 刷新；场景二文档导入 → 独立研究 → 返回 → 重启恢复；场景三稍后再学保存 → 栏目 → 返回重开 → 刷新；场景四刷新幂等 + 材料范围 + 无演示标记）。全部四条场景通过，回答 / 分析 / 研究均为真实 AI 生成，真实模型状态可见，390px 响应式不溢出，控制台与网络干净，SQLite 一致性验证通过，文本型 PDF canvas 绑定警告为既有已知限制。"核心流程 MVP 可体验"表述已正式生效。
...
因此，当前版本证明本地运行、研究会话、文件导入、阅读、恢复底座、选区智能窗口、深入研究全链路与稍后再学保存 / 列表 / 来源返回均可用，核心闭环主链路已端到端连通并通过真实模型验收。
...
阶段 H 并行三切片合并后（2026-07-30，§3.37）建立四级基线；选区交互修订一·A（§3.38）为纯前端切片按二级 + e2e 验证；修订一·B（2026-07-31，§3.39，契约改动）重跑四级全量；修订一·C（2026-07-31，§3.40，纯前端）按二级 + e2e 验证，当前基线：

| 范围 | 结果 |
| --- | --- |
| Node 单元与集成测试 | 302/302 通过（dist-tests 287 + ralph 脚本 15） |
| WebUI 客户端测试 | 260/260 通过（修订一·C 后；较修订一·B +4 胶囊淡出与选区活动用例） |
| Playwright e2e（假模型，真实 Chromium） | 53/53 通过（chromium 50 + chromium-nomodel 3），2 项暂停（稍后再学 UI 入口，待修订二替换为标记流程） |
...
- H4a（§3.37）起旧选区智能窗口退役，替换为引用胶囊与双模发送（在此追问 / 深入研究这段）；修订一·A（§3.38）起引用改为选区上方浮动胶囊显式触发，引用态与原生选区解耦（死循环修复），Escape 不再关闭选区或胶囊，取消方式唯一为点击选取以外区域；修订一·B（§3.39）起最短字符限制全层退役（非空即有效，字数上限不变）；修订一·C（§3.40）起 `?sel=` 恢复后浮动胶囊呈现在高亮标记上方、引用后焦点回归输入框，窄屏钳制与上方空间不足翻转有端到端证明，浮动胶囊键盘可达，胶囊出现 / 消失带轻过渡（减弱动效环境关闭），修订一全部完成；旧面板「稍后再学」UI 入口暂时移除，后端端点保留，新入口待修订二（issues #8/#12/#13）；术语检测（H3a）与父链上下文（H5a）为地基层，尚未接入提示词与渲染；
- 离线演示回答不是产品能力证据；
...
- 修订一·C（胶囊边界场景与恢复，issue #11）：`4d8ad75`（`feat(web): 胶囊边界场景与恢复`，提交信息含验证证据）。
...
- 修订一（A/B/C）全部完成；修订二（issues #8/#12/#13：【标记】按钮与【引用】共用浮动胶囊、笔记输入框、标记列表与返回原文、旧"稍后再学"数据迁移）接续实施；
- 2 项暂停 e2e（稍后再学 UI 入口）待修订二以标记流程替换后恢复。
