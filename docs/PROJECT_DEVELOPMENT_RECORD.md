...
| 真实供应商联网验收 | 未完成；产品决策已调整为"默认关闭 + 输入框开关"，当前 4 家已配置供应商均不支持原生联网，待开关改造完成并配置 OpenAI / Gemini / Anthropic（或已接入的 Mimo 自有协议）后验收 |
...
- **真实验收状态**：当前已配置的 DeepSeek 与 Mimo 在代码中均标记为 `webGrounding: "unsupported"`，无法触发供应商联网路径；用户确认 Mimo 使用模型商自有联网搜索服务，协议待确认接入。产品决策已更新为"用户主动场景默认关闭、输入框开关控制"，选区分析维持始终不联网，其他 AI 生成场景由任务类型自行判断。需在开关改造完成且至少配置一家支持联网的模型（OpenAI / Gemini / Anthropic / 已接入的 Mimo）后执行真实联网验收。

## 7. 下一阶段开发方向

阶段 E（可信研究能力）全栈实现已提交（`0343c56`），但产品决策已调整为"用户主动场景默认关闭、输入框联网开关控制"（详见 §3.33）。当前后端保存的 4 个 profile 均不支持原生联网；真实验收需先完成开关入口改造，再配置至少一家支持联网的模型（OpenAI / Gemini / Anthropic，或已接入的 Mimo 自有协议）。
...
- 阶段 E 收尾从"待真实验收"调整为"待用户可控开关改造后真实验收"；
- 阶段 H（统一研究节点树）不受此调整阻塞，但 H 中的"就地追问"和"节点生长"输入框也需要继承同一开关语义。
