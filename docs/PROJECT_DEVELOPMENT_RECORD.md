...
进行中：下一阶段方向见 §7。核心流程 MVP 已可体验，当前验证基线：Node 测试 137/137，WebUI 测试 194/194，Playwright Chromium 40/43（3 项失败为真实模型验收 spec 需真实密钥 + 1 项 e2e 数据库隔离干扰，均为已知限制）。
...
新增提交 `6ee4175`（文件发起研究 + 阅读页聊天）和 `ff9823d`（关闭 Collector 按钮），WebUI 测试从 185 升至 194。
...
- 启动器不会强制结束身份无法验证的进程，这类情况需要用户先关闭旧服务；现在可以通过顶栏"关闭 Collector"按钮从界面主动关闭服务。
...
核心流程 MVP 已经端到端连通并通过真实模型四场景验收。后续迭代方向：

1. 联网搜索、行内引用和按需验证（DEVELOPMENT_START.md §5.2 可信研究支撑）；
