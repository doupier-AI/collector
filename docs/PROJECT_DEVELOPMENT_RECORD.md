...
进行中：切片 D1（稍后再学保存与列表后端，迁移 v19）已完成并提交，四级验证全部通过（全量构建、Node 测试 198/198、项目检查）。下一切片为 D2（稍后再学栏目与来源返回 WebUI），交接语见 3.17 末尾。
...
当前实现已经连通核心闭环的大部分主链路：阅读当前内容（Chat 回答与导入文档）→ 手动选择文字 → 选区质量提示 → 选区智能窗口（原文即时保存、AI 分析逐字段呈现、失败保留原文可重试）→ 深入研究二选一（沿当前内容建立分支或以选区开启独立会话）→ 第一轮真实生成、材料范围说明、来源条与返回原选区高亮。
...
其中稍后再学的后端与数据底座已在切片 D1 完成：项目保存（确定性概括默认值、一至五星优先级、pending / done 状态）、创建幂等、列表联接选区文本与来源说明、priority / summary / status 更新均已通过自动化验证；保存、展示和返回不依赖 AI。用户可见的窗口保存操作、稍后再学栏目与来源返回由切片 D2 连接。

因此，当前版本证明本地运行、研究会话、文件导入、阅读、恢复底座、选区智能窗口、深入研究全链路和稍后再学后端可用，但不代表产品核心流程 MVP 已经完成。下一阶段边界见 `DEVELOPMENT_START.md`，切片状态见 `MVP_IMPLEMENTATION_PLAN.md`。
